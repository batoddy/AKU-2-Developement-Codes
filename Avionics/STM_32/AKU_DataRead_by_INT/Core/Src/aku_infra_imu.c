...
IMU_Device bno055 = {
	...
	//.int_en = BNO055_INT_EN, // page 1
	//.int_sta = BNO055_INT_STATUS
};
...
// -------------------- USER CODE 1 BEGIN ---------------------
void imu_read_register(uint8_t reg, uint8_t *data, uint8_t len)
{ // read from register func.
	bno055_readData(reg, data, len);
}

void imu_write_register(uint8_t reg, uint8_t data)
{ // write to register func.
	bno055_writeData(reg, data);
}
...
void config_IMU() // write imu configuration func.
{
	...
	HAL_GPIO_WritePin(IMU_RST_GPIO_Port, IMU_RST_Pin, GPIO_PIN_SET); // reset_bno
...
IMU *read_IMU()
{
	...
	imu.prev_tick = imu.tick;
	imu.prev_accel = imu.accel;
	imu.prev_z_velocity = imu.z_velocity;
	...
	imu.data_num++;
	if (imu.data_num > 18446744073709551615-5)
		imu.data_num = 0;
